...
function mergeReasons(reasons) {
  ...
  let kept = reasons.filter((r) => {
    ...
    if (r._category === "profanity" && r.span)
      return !rulePII.some((rr) => rr.span[0] <= r.span[0] && r.span[1] <= rr.span[1]);
    return true;
  });
...
export function moderate(raw, { neural = null, config = {}, lowercase = false } = {}) {
  ...
  const material = skeleton(raw) !== raw;
  const { normalized, map } = normalize(raw, { lowercase });
  let flags = detect(raw, { enabled, detectorOpts });
