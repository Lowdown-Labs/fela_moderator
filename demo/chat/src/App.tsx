...
export function App() {
  ...
  const onFlagged = (findings: Finding[]) => new Promise<Decision>((resolve) => setDialog({ findings, resolve }));
      ...
      <main className="flex-1 space-y-2 overflow-y-auto p-4">
        {msgs.map((m, i) => (
          <div key={i} className={m.from === "me" ? "flex justify-end" : "flex justify-start"}>
            <div
              className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm ${m.from === "me" ? "bg-emerald-500 text-white" : "bg-white text-slate-800 ring-1 ring-slate-200"}`}
            >
              {m.text}
            </div>
          </div>
        ...
        <div className="flex items-end gap-2">
          <div className="flex-1">
            <ModeratedTextarea
              ...
              classNames={{
                input:
                  "resize-none rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-400",
              }}
            />
          </div>
          <button
