...
```sh
npm i @lowdown/moderate onnxruntime-react-native
# bundle the model as an asset: android/app/src/main/assets/moderator.onnx (or the .tflite)
```
...
import { createModerator } from "@lowdown/moderate/model/runner.mjs";
import { check } from "@lowdown/moderate";
import config from "@lowdown/moderate/config.json";
...
`react/ModeratedTextarea` works in RN too (swap the `<textarea>` for `<TextInput>`); pass `moderator={moderate}`:

```jsx
<ModeratedTextarea moderator={moderate} onGate={g => setCanSave(!g.blocked)} />
```
