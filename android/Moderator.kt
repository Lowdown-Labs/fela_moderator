...
// Android reference port of the submit-time gate. The correctness-critical parts — byte tokenization,
// the byte->UTF-16 offset mapping for PII redaction, BIO span merging, thresholds, and the hybrid gate
// — mirror sdk/reference/*.mjs (which are unit-tested). The TFLite call is an injected interface so
// this file has no hard TFLite dependency; wire it to LiteRT/tflite Interpreter in your app.
//
...
/** Raw model logits for one 512-window. Supply via your TFLite Interpreter. */
data class ModelOutput(val jigsaw: FloatArray, val piiArgmax: IntArray /* len 512 */)

/** entity + native String (UTF-16) offsets, so text.substring(start, end) is the PII. */
...
class Moderator(private val cfg: ModConfig, private val runModel: (IntArray) -> ModelOutput) {
    ...
    /** Byte-tokenize: [CLS, b0, b1, ...] padded to 512; returns ids + the byte index per token. */
    ...
    /** For every UTF-8 byte offset, the containing char's [startUtf16, endUtf16). Kotlin String is UTF-16. */
    private fun charBoundsByByte(text: String): Pair<IntArray, IntArray> {
        val starts = ArrayList<Int>(); val ends = ArrayList<Int>()
        var u = 0; var i = 0
        while (i < text.length) {
            val cp = text.codePointAt(i)
            val units = Character.charCount(cp)                 // 1 or 2 UTF-16 units
            val bl = String(Character.toChars(cp)).toByteArray(Charsets.UTF_8).size  // 1..4 bytes
    ...
    private fun modelPii(argmax: IntArray, byteOf: IntArray, text: String): List<PiiSpan> {
        ...
        flush()
        return out.filter { it.entity in MODEL_OWNED }          // regex owns structured PII
    }
...
/** FOSS structured-PII checkers (email/phone/SSN/card/IP) — regex + Luhn, mirroring checkers.mjs. */
object Checkers {
    ...
    fun structuredPII(text: String): List<PiiSpan> {
        ...
        // drop overlaps (first-start / longer wins)
